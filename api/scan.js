...
//
// What was wrong: the old lineup called retired models first
// (Groq qwen3.6 = "does not exist", Gemini 2.5 = retired) and accepted the
// first text blindly — including empty/garbled JSON, shown as blank results.
// Verified-good lineup (probed live): Gemini gemini-3.5-flash then
// gemini-3.6-flash, then keyless Pollinations; Cloudflare Scout first when
// CLOUDFLARE_API_TOKEN + CLOUDFLARE_ACCOUNT_ID are configured.
// Only the first candidate that parses into >=2 filled keys is returned.
// Total worst case fits Vercel's ~60s function limit. No env changes needed:
// Scout/Pollinations activate only if their (optional) config exists.
